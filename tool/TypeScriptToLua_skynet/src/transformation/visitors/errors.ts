...
import { getAwaitFeature, wrapInAsyncAwaiter } from "./async-await";
...
const transformAsyncTry: FunctionVisitor<ts.TryStatement> = (statement, context) => {
    ...
    if (statement.catchClause) {
        ...
        const promiseAwait = transformLuaLibFunction(context, getAwaitFeature(context), statement, catchCall);
        result.push(lua.createExpressionStatement(promiseAwait, statement));
    } else {
        // await ____try
        const promiseAwait = transformLuaLibFunction(context, getAwaitFeature(context), statement, awaiterIdentifier);
        result.push(lua.createExpressionStatement(promiseAwait, statement));
    }
