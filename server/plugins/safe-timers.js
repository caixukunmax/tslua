...
const plugin = {
    ...
    visitors: {
        ...
        [ts.SyntaxKind.CallExpression]: (node, context) => {
            ...
            const runtimeAccess = ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier("runtime"), "timer");
            const methodAccess = ts.factory.createPropertyAccessExpression(runtimeAccess, methodName);
            // 构建新参数列表
            ...
            // 创建新的调用表达式
            const newCall = ts.factory.createCallExpression(methodAccess, undefined, newArgs);
            // 转换新的调用
